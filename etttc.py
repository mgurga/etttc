...
while True:
    if regORlog == "":
        ...
        loginjson = '{"call": "login", "usernm": "' + \
            usernm + '", "passwd": "' + passwd + '"}'
...
def drawterminal():
    ...
    print(term.move_xy(0, 0) + providername + ", " + connectedtodisplay + ", loaded messages up to: " +
          str(currentchannelmsgnum) + "(" + str(totalmessagesinchannel) + " latest)")
        ...
        if not (currentserver == "" or currentchannel == ""):
            ...
            if servresponsejson["resp"] == True:
                ...
                if not end == 0:
                    ...
                    serversocket.send(
                        (json.dumps(messagetotal)).encode(encoding))
                    servresponse = serversocket.recv(4096)
                    servresponsejson = json.loads(
                        servresponse.decode(encoding))
                    ...
                    if servresponsejson["resp"] == True:
                        ...
                        err("unable to get messages, " +
                            servresponsejson["reason"])

            else:
                err("unable to get total messages, " +
                    servresponsejson["reason"])
        time.sleep(0.75)
...
def err(err):
    if len(messagebuffer) > 1:
        messagebuffer[len(
            messagebuffer) - 1] = messagebuffer[len(messagebuffer) - 1] + "  !!!" + err + "!!!"
        drawterminal()
...
while True:
    ...
    try:
        if usermsg == "/createserver":
            ...
            if servresponsejson.get("call") == None:
                if servresponsejson["resp"] == True:
                    ...
                    err("couldn't get list of servers, " +
                        servresponsejson["reason"])
        ...
        elif usermsg == "/setserverimg" and currentserver != "":
            imgb64 = input("copy and paste image in base64 here: ")

            setimgreq = json.loads("{}")
            setimgreq["call"] = "setserverimage"
            setimgreq["servername"] = currentserver
            setimgreq["img"] = imgb64
            setimgreq["usernm"] = usernm
            setimgreq["passwd"] = passwd

            serversocket.send((json.dumps(setimgreq)).encode(encoding))

            servresponse = serversocket.recv(4096)
            servresponsejson = json.loads(servresponse.decode(encoding))

            if servresponsejson["resp"] == True:
                log("server image has been set")
            else:
                err("couldn't set image, " + servresponsejson["reason"])

        elif usermsg == "/getserverimg" and currentserver != "":
            setimgreq = json.loads("{}")
            setimgreq["call"] = "getserverimage"
            setimgreq["servername"] = currentserver
            setimgreq["usernm"] = usernm
            setimgreq["passwd"] = passwd
            ...
            servresponse = serversocket.recv(16384)
            servresponsejson = json.loads(servresponse.decode(encoding))

            if servresponsejson["resp"] == True:
                log("got server image")
                log(servresponsejson["img"])
            else:
                err("couldn't get image, " + servresponsejson["reason"])
